        ...
        <div class="container-fluid p-0">
            <div class="panel panel-primary w-100">
                ...
                <span class="d-flex flex-column justify-content-center align-items-center">
                    ...
                    <form action="{{ route('Admin.searchLink') }}" method="GET" class="text-dark d-flex row justify-content-center align-items-center mb-4 search no-gutters">
                ...
                <div class="panel-body d-flex flex-column justify-content-center align-items-center">
                    <div class="table-responsive">
                        <table class="table text-center table-responsive" dir="rtl">
                            <thead>
                            <a href="{{route('Admin.Links.create')}}" class="btn btn-block btn-info">لینک سازی<i class="material-icons">add_circle_outline</i></a>
                            <tr>
                            ...
                            @foreach($items as $item)
                                <tr class="text-center">
                                    ...
                                    <td>{{$item->PName}}</td>
                                    <td>{{$item->ProName1}}</td>
                                    <td>@if(!$item->ProName2 == "") {{$item->ProName2}} @else خالی @endif</td>
                                    <td>@if(!$item->ProName3 == "") {{$item->ProName3}} @else خالی @endif</td>
                                    <td><a href="{{ route('Admin.Links.show', $item->id) }}" class="btn btn-info"><i class="material-icons">visibility</i></a></td>
                                    <td><a href="{{ route('Admin.Links.edit', $item->id) }}" class="btn btn-warning"><i class="material-icons">edit</i></a></td>
                                    <td>
                                        {{ html()->form('DELETE', route('Admin.Links.destroy', $item->id))->open() }}
                                        <button class="btn btn-danger @if($item->Storage <= $item->OrderDot & !$item->Storage == "") btn-dark @endif"><i class="material-icons">delete</i></button>
                                        {{ html()->form()->close() }}
                                    </td>
                                </tr>
                            @endforeach
                    ...
                    <!-- Pagination -->
                    <span dir="ltr">
                        <a href="{{$items->previousPageUrl()}}" class="btn btn-light"><i class="material-icons text-dark">arrow_back</i></a>
                        @for($i=1;$i<=$items->lastPage();$i++)
                            <a href="{{$items->url($i)}}" class="btn btn-light page-item">{{$i}}</a>
                        @endfor
                        <a href="{{$items->nextPageUrl()}}" class="btn btn-light"><i class="material-icons text-dark">arrow_forward</i></a>
                    </span>
                </div>
            </div>
        </div>
