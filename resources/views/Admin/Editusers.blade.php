            ...
            <div>
                <div class="panel panel-primary">
                    <div class="panel-heading clearfix mb-4 d-flex flex-column justify-content-center align-items-center">
                        ...
                        <p>از طریق این صفحه میتوانید تعداد کاربران موجود در سایت را حذف و یا اضافه کنید،همچنین میتوانید مشخصات آنها را نیز تغییر بدهید. لطفا در نظر داشته باشید برای تغییر بر روی دکمه مداد کلیک کنید و برای حذف بر روی دکمه سطل آشغال کلیک کنید.</p>
                    </div>
                    <!-- panel body -->
                        ...
                        <table class="table text-center" dir="rtl">
                            ...
                            @foreach($items as $item)
                                <tr>
                                    ...
                                    <td>{{$item -> Lastname}}</td>
                                    <td>{{$item -> email}}</td>
                                    <td>@if($item->Role !== null){{$item -> Role}}@else تعیین نشده @endif</td>
                                    <td>{{$item -> created_at}}</td>
                                    <td>{{$item -> updated_at}}</td>
                                    <td><a href="{{ route('Admin.Editusers.edit', $item->id) }}" class="btn btn-warning">
