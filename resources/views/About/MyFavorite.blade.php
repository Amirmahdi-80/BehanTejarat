            ...
            <div class="col-md-12">
                <div class="panel panel-primary">
                    <div
                        ...
                        <img src="{{asset('assets/images/Favorite.png')}}" alt="علاقه مندی های من" class="img2">
                    ...
                    <div class="panel-body d-flex flex-column justify-content-center align-items-center col-12">
                        <table class="table text-center" dir="rtl">
                            <thead>
                            <a href="{{route('Admin.Favorite.create')}}" class="btn btn-block btn-info">افزودن علاقه مندی<i
                            ...
                            @foreach($items as $item)
                                <tr class="text-center">
                                    ...
                                    <td data-toggle="tooltip" data-placement="bottom"
                                        title="برای دانلود فایل کلیک کنید">@if(!$item->Pic=="")
                                            <a href="{{ route("files.show", $item->Pic) }}"><img
                                                    class="avatar5 img-circle"
                                                    src="{{ route("files.show", $item->Pic) }}"
                                                    alt="عکس" title="عکس"
                                                    style="border-radius: 5px"></a>
                                        @else
                                            <span>خالی</span>
                                        @endif</td>
                                    <td><a href="{{ route('Admin.Favorite.edit', $item->id) }}" class="btn btn-warning">
                                            <i class="material-icons">edit</i>
                                        </a></td>
                                    <td>
                                        {{ html()->form('DELETE', route('Admin.Favorite.destroy', $item->id))->open() }}
