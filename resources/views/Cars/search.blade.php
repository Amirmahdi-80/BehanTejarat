        ...
        <div class="row no-gutters">
            <div class="panel panel-primary">
                ...
                <span class="d-flex flex-column justify-content-center align-items-center">
                    ...
                    <form action="{{ route('Admin.search') }}" method="GET" class="text-dark d-flex row justify-content-center align-items-center mb-4 search no-gutters">
                    ...
                    <form action="{{ route('Admin.filter') }}" method="GET" class="text-dark d-flex row justify-content-center align-items-center mb-4 search no-gutters No4">
                ...
                <!-- panel body -->
                <div class="panel-body d-flex flex-column justify-content-center align-items-center w-100 overflow-auto" id="printdivcontent">
                    @if($items->isNotEmpty())
                            ...
                            <tr class="text-center" dir="ltr">
                                ...
                                <td>
                                    @if($search === null)
                                        @foreach($cars as $car)
                                            {{ $car->CarName }}
                                            @if(!$loop->last) {{-- Add comma after all but the last item --}}
                                            ,
                                            @endif
                                        @endforeach
                                    @else
                        ...
                        @foreach ($items as $item)
                            <table class="table text-center table-responsive mb-2" dir="rtl" style="border-bottom: 3px solid #333">
                                <thead class="bg-info">
                                <tr>
                                ...
                                <tr class="text-center">
                                    ...
                                    @foreach($drivers as $driver)
                                        @if($driver->DriverName == $item->driver_id)
                                            <td data-toggle="tooltip" data-placement="bottom" title="برای دانلود عکس کلیک کنید">
                                                @if(!$driver->image == "")
                                                    <a href="{{ route('files.show', $driver->image) }}"><img class="avatar img-circle" src="{{ route('files.show', $driver->image) }}" alt="عکس راننده خودرو" title="عکس راننده خودرو" style="border-radius: 5px;width: 50px; height: 50px;"></a>
                                                @else
                                                    <span>خالی</span>
                                                @endif
                                            </td>
                                    ...
                                    @foreach($cars as $car)
                                        @if($car->CarName == $item->car_id)
                                            <td data-toggle="tooltip" data-placement="bottom" title="برای دانلود عکس کلیک کنید">
                                                @if(!$car->image == "")
                                                    <a href="{{ route('files.show', $car->image) }}"><img class="avatar img-circle" src="{{ route('files.show', $car->image) }}" alt="عکس خودرو" title="عکس خودرو" style="border-radius: 5px;width: 50px; height: 50px;"></a>
                                                @else
                                                    <span>خالی</span>
                                                @endif
                                            </td>
                    ...
                    @else
                        <div>
                            <h2 class="text-dark mt-5 d-flex row justify-content-center align-items-center" style="gap: 10px">نتیجه ای یافت نشد<i class="material-icons text-dark">mood_bad</i></h2>
                        </div>
                    @endif
                </div>
            </div>
